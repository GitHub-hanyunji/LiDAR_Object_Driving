...
void LiDARDetectNode::image_callback(sensor_msgs::msg::CompressedImage::SharedPtr msg)
{
    ...
    // CompressedImage 디코딩 -> OpenCV Mat
    // msg->data 는 std::vector<uint8_t> (jpg/png 바이너리)
    ...
    // 기존 코드와 동일하게 사용 가능
    if (video_key) writer.write(frame);
    ...
    Draw(roi, stats, centroids, labels, stats.rows, roi_center);
    Draw(frame, stats, centroids, labels, stats.rows, p_center);
    ...
    if (kbhit()) {
        ...
        if (ch == 's') {
            ...
            writer.open("output.mp4", fourcc, 10.0,
                        cv::Size(frame.cols, frame.rows), true);
            if (writer.isOpened()) {
                video_key = true;
                mode = true;
                RCLCPP_INFO(this->get_logger(), "녹화 시작");
            }
        } else if (ch == 'r') {
            std::cout << "라이다 범위 입력: ";
            std::cin >> range;
    ...
    vel_msg.x = mode ? leftvel  : 0.0;
    vel_msg.y = mode ? rightvel : 0.0;
    vel_msg.z = 0.0;
